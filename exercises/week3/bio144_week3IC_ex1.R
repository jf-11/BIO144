...
ggplot(transformed_data) + aes(log_health,log_child) +
  geom_point(aes(color=continent)) + geom_smooth(method='lm',formula = y~x,color='red') +
  labs(x='log of health care',y='log of child mortality') + 
  #annotate(geom='text',x=3.666785,y=0.6232493,label='Switzerland', size= 4)
  geom_text(data=transformed_data,label=transformed_data$country,color='black',size=3,vjust=2)
