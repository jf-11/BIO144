...
filtered <- class_RTs %>%
  filter(Pref_Reaction_time > 50 & Pref_Reaction_time < 500)
View(filtered)
sum(class_RTs_filtered != filtered)
class_RTs_filtered %>%
ggplot() +
  geom_jitter(mapping = aes(x=Gender, y=Pref_Reaction_time,color=Pref_Reaction_time),
              width=0.05)
#######################################################
...
library(ggthemes)
ggplot(data=class_RTs_filtered, aes(x=Gender, y=Pref_Reaction_time,color=Gender,alpha=Pref_Reaction_time)) +
  geom_boxplot() +
  geom_jitter(width= 0.1) +
  labs(alpha='Reaction Time',title='Reaction time Males vs. Females')+
