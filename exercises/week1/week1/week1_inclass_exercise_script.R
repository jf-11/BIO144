...
class(class_RTs$Pref_Reaction_time_1)
...
ggplot(data=class_RTs, aes(x=Pref_Reaction_time_1)) +
  geom_histogram(binwidth = 30)

## Now make a figure containing two histograms histograms (i.e. two "facets"), one for each gender
ggplot(data=class_RTs, aes(x=Pref_Reaction_time_1)) +
  geom_histogram(binwidth=30) +
  facet_grid(.~Gender) # if gender and ~ changed then cols and not rows.

## And a box and whisker plot
ggplot(data=class_RTs, aes(x=Gender, y=Pref_Reaction_time_1)) +
  geom_boxplot()

## Or just the data points (with some jitter, to separate overlapping points):
ggplot(data=class_RTs, aes(x=Gender, y=Pref_Reaction_time_1,color=Gender,alpha=Pref_Reaction_time_1)) +
...
class_RTs_filtered <- class_RTs %>%
  filter(Pref_Reaction_time_1 > 500 | Pref_Reaction_time_1 < 50)
str(class_RTs_filtered)
class_RTs_filtered %>%
  ggplot() +
  geom_jitter(mapping = aes(x=Gender, y=Pref_Reaction_time_1),width=0.05)
#######################################################
...
ggplot(data=class_RTs, aes(x=Gender, y=Pref_Reaction_time)) +
  ...
  labs(title='Reaction time Female vs Male') + 
  theme_minimal()
